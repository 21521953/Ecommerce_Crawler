...
def scrape_data(base_url):
    ...
    headers = ['Product Title', 'Brand', 'Price', 'Regular Price', 'Discount %', 'Image', 'Product Evaluation',
               'Product_score_average', 'Detail', 'Thương hiệu', 'SKU', 'Dạng sản phẩm', 'Mẫu mã',
    ...
    with open('crawler_stock_test2.csv', mode='a', newline='', encoding='utf-8-sig') as file:
        ...
        while True:
            ...
            try:
                ...
                pagination_num = int(pagination.find_elements(By.CLASS_NAME, "ant-pagination-item")[-1].text)
                print("Page Total:", pagination_num)
            except Exception as e:
                print(e)

            if current_page > pagination_num:
            ...
            for index, productLink in enumerate(listProductLink):
                ...
                try:
                    ...
                    if is_product_seen(productTitle, existing_titles):
                        ...
                        productEvaluation = browser.find_element(By.CLASS_NAME,
                                                                 "pdp-review-summary__link").text
                        productPrice = browser.find_element(By.CSS_SELECTOR, ".pdp-price_type_normal").text
                        productPrice = re.match(r'^[\d|\.|\,]+', productPrice).group()

                        try:
                            productRegularPrice_element = browser.find_element(By.CLASS_NAME,
                        ...
                        try:
                            DiscountPercent_element = browser.find_element(By.CLASS_NAME,
                                                                           "pdp-product-price__discount")
                            DiscountPercent = DiscountPercent_element.text
                        except NoSuchElementException:
                            DiscountPercent = 0

                        productImage = browser.find_element(By.CLASS_NAME, "pdp-mod-common-image").get_attribute(
                            "src")
